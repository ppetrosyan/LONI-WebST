...
public class GridTab {
	...
	public Tab setTab() {
		...
		gridEngineNativeSpec = new TextItem("gridEngineNativeSpec",
				"Grid engine native specification");
		gridEngineNativeSpec.setDisabled(true);

		jobNamePrefix = new TextItem("jobNamePrefix", "Job name prefix");
		...
		complexResourceAttributes = new TextItem("complexResourceAttributes",
		...
		String[] policyArray = { "Disallow all variables, except specified",
		...
		gridVariablesPolicyText = new TextItem("gridVariablesPolicyText",
				"Specified variables");
		...
		maxParallelThreads = new SpinnerItem("maxParallelThreads",
		...
		maxNumResubmissions = new SpinnerItem("maxNumResubmissions",
		...
		dynamicFormGeneral.setItems(new FormItem[] { gridEngineNativeSpec,
				jobNamePrefix, jobSubmissionQueue, complexResourceAttributes,
				sgeMem, gridVariablesPolicy, gridVariablesPolicyText,
				prefixBeforeVar, maxParallelThreads, maxNumResubmissions,
				totalNumSlots, knownNumber, getFromCommand });
		...
		dynamicFormArrayJob.setItems(new FormItem[] { enableArrayJob, chunks,
				fileStat, chunkSize, increaseChunkSize, maxChunkSize });
		...
		String[] pluginArray = { "JGDI", "DRMAA", "Customized" };
		...
		dynamicFormGridPlugin.setItems(new FormItem[] { gridPlugin, jarFiles,
				jarFilesClass, restartService, gridPort, memItem, jarFile,
		...
		gridPassword = new PasswordItem("gridPassword", "Password");
		gridPassword.setDisabled(true);

		dynamicFormGridAccounting = new DynamicForm();
		dynamicFormGridAccounting.setItems(new FormItem[] { gridAcctURL,
	...
	public void parseGridXML(Document doc) {
		...
		// enable grid section
		Node enableGrid = doc.getElementsByTagName("EnableGrid").item(0);
		if (enableGrid != null) {
			Boolean enableGridVal = Boolean.valueOf(enableGrid.getFirstChild()
					.getNodeValue());
		...
		// general section
		Node gridEngineNativeSpec = doc.getElementsByTagName(
				"GridEngineNativeSpec").item(0);
		if (gridEngineNativeSpec != null) {
			String gridEngineNativeSpecVal = gridEngineNativeSpec
					.getFirstChild().getNodeValue();
			dynamicFormGeneral.getItem("gridEngineNativeSpec").setValue(
					gridEngineNativeSpecVal);
		}

		Node jobNamePrefix = doc.getElementsByTagName("JobNamePrefix").item(0);
		if (jobNamePrefix != null) {
			String jobNamePrefixVal = jobNamePrefix.getFirstChild()
					.getNodeValue();
			dynamicFormGeneral.getItem("jobNamePrefix").setValue(
					jobNamePrefixVal);
		}

		Node jobSubmissionQueue = doc
				.getElementsByTagName("JobSubmissionQueue").item(0);
		if (jobSubmissionQueue != null) {
			String jobSubQueueVal = jobSubmissionQueue.getFirstChild()
					.getNodeValue();
			dynamicFormGeneral.getItem("jobSubmissionQueue").setValue(
					jobSubQueueVal);
		}

		Node complexResourceAttributes = doc.getElementsByTagName(
				"ComplexResourceAttributes").item(0);
		if (complexResourceAttributes != null) {
			String complexResAttrVal = complexResourceAttributes
					.getFirstChild().getNodeValue();
			dynamicFormGeneral.getItem("complexResourceAttributes").setValue(
					complexResAttrVal);
		}

		Node sgeMem = doc.getElementsByTagName("ReportSGE").item(0);
		if (sgeMem != null) {
			Boolean sgeMemVal = Boolean.valueOf(sgeMem.getFirstChild()
					.getNodeValue());
			dynamicFormGeneral.getItem("sgeMem").setValue(sgeMemVal);
		}

		Node gridVariablesPolicy = doc.getElementsByTagName(
				"GridVariablesPolicy").item(0);
		if (gridVariablesPolicy != null) {
			String gridVarPolicyVal = gridVariablesPolicy.getFirstChild()
					.getNodeValue();
			dynamicFormGeneral.getItem("gridVariablesPolicy").setValue(
					gridVarPolicyVal);
		}

		Node gridVariablesPolicyText = doc.getElementsByTagName(
				"SpecifiedVariables").item(0);
		if (gridVariablesPolicyText != null) {
			String gridVarPolicyTextVal = gridVariablesPolicyText
					.getFirstChild().getNodeValue();
			dynamicFormGeneral.getItem("gridVariablesPolicyText").setValue(
					gridVarPolicyTextVal);
		}

		Node prefixBeforeVar = doc.getElementsByTagName("PrefixBeforeVar")
				.item(0);
		if (prefixBeforeVar != null) {
			String prefixBeforeVarVal = prefixBeforeVar.getFirstChild()
					.getNodeValue();
			dynamicFormGeneral.getItem("prefixBeforeVar").setValue(
					prefixBeforeVarVal);
		}

		Node maxParallelThreads = doc.getElementsByTagName("MaxNumParallelSub")
				.item(0);
		if (maxParallelThreads != null) {
			int maxParThreadsVal = Integer.parseInt(maxParallelThreads
					.getFirstChild().getNodeValue());
			dynamicFormGeneral.getItem("maxParallelThreads").setValue(
					maxParThreadsVal);
		}

		Node maxNumResubmissions = doc.getElementsByTagName(
				"MaxNumResubmissions").item(0);
		if (maxNumResubmissions != null) {
			int maxNumResubVal = Integer.parseInt(maxNumResubmissions
					.getFirstChild().getNodeValue());
			dynamicFormGeneral.getItem("maxNumResubmissions").setValue(
					maxNumResubVal);
		}

		Node totalNumSlots = doc.getElementsByTagName("TotalNumSlots").item(0);
		if (totalNumSlots != null) {
			String totalNumSlotsVal = totalNumSlots.getFirstChild()
					.getNodeValue();
			dynamicFormGeneral.getItem("totalNumSlots").setValue(
					totalNumSlotsVal);
			toggleTotalNumSlots(false);

			Node totalNumSlotsTextVal = doc.getElementsByTagName(
					"TotalNumSlotsVal").item(0);
			if (totalNumSlotsVal != null) {
				if (totalNumSlotsVal.equals("Known number")) {
					int knownNumberVal = Integer.parseInt(totalNumSlotsTextVal
							.getFirstChild().getNodeValue());
					dynamicFormGeneral.getItem("knownNumber").setValue(
							knownNumberVal);
				} else if (totalNumSlotsVal.equals("Get from command")) {
					String getFromCommandVal = totalNumSlotsTextVal
							.getFirstChild().getNodeValue();
					dynamicFormGeneral.getItem("getFromCommand").setValue(
							getFromCommandVal);
				}
			}
		}
		...
		// array job section
		Node enableArrayJob = doc.getElementsByTagName("EnableArrayJob")
				.item(0);
		if (enableArrayJob != null) {
			Boolean arrayJobVal = Boolean.valueOf(enableArrayJob
					.getFirstChild().getNodeValue());
			dynamicFormArrayJob.getItem("enableArrayJob").setValue(arrayJobVal);
			toggleArrayJob(arrayJobVal);
		}

		Node chunks = doc.getElementsByTagName("BreakIntoChunks").item(0);
		if (chunks != null) {
			int chunksVal = Integer.parseInt(chunks.getFirstChild()
					.getNodeValue());
			dynamicFormArrayJob.getItem("chunks").setValue(chunksVal);
		}

		Node fileStat = doc.getElementsByTagName("FileStatTimeout").item(0);
		if (fileStat != null) {
			int fileStatVal = Integer.parseInt(fileStat.getFirstChild()
					.getNodeValue());
		...
		if (chunkSize != null) {
			int chunkSizeVal = Integer.parseInt(chunkSize.getFirstChild()
					.getNodeValue());
			dynamicFormArrayJob.getItem("chunkSize").setValue(chunkSizeVal);
		}

		Node increaseChunkSize = doc.getElementsByTagName(
				"GraduallyIncreaseChunkSize").item(0);
		if (increaseChunkSize != null) {
			Boolean increaseChunkSizeVal = Boolean.valueOf(increaseChunkSize
					.getFirstChild().getNodeValue());
			dynamicFormArrayJob.getItem("increaseChunkSize").setValue(
					increaseChunkSizeVal);
		...
		Node maxChunkSize = doc.getElementsByTagName("MaxChunkSize").item(0);
		if (maxChunkSize != null) {
			int maxChunkSizeVal = Integer.parseInt(maxChunkSize.getFirstChild()
					.getNodeValue());
			dynamicFormArrayJob.getItem("maxChunkSize").setValue(
					maxChunkSizeVal);
		}

		// grid plugin section
		Node gridPlugin = doc.getElementsByTagName("GridPlugin").item(0);
		if (gridPlugin != null) {
		...
		Node jarFiles = doc.getElementsByTagName("JarFiles").item(0);
		if (jarFiles != null) {
			String jarFilesVal = jarFiles.getFirstChild().getNodeValue();
			dynamicFormGridPlugin.getItem("jarFiles").setValue(jarFilesVal);
		}

		Node jarFilesClass = doc.getElementsByTagName("JarFilesClass").item(0);
		if (jarFilesClass != null) {
			String jarFilesClassVal = jarFilesClass.getFirstChild()
					.getNodeValue();
			dynamicFormGridPlugin.getItem("jarFilesClass").setValue(
					jarFilesClassVal);
		}

		Node restartService = doc.getElementsByTagName("RestartService")
				.item(0);
		if (restartService != null) {
			Boolean restartServiceVal = Boolean.valueOf(restartService
					.getFirstChild().getNodeValue());
			dynamicFormGridPlugin.getItem("restartService").setValue(
					restartServiceVal);
			toggleRestartService(restartServiceVal);
		}

		Node gridPort = doc.getElementsByTagName("GridPort").item(0);
		if (gridPort != null) {
			int gridPortVal = Integer.valueOf(gridPort.getFirstChild()
					.getNodeValue());
			dynamicFormGridPlugin.getItem("gridPort").setValue(gridPortVal);
		}

		Node memItem = doc.getElementsByTagName("GridMemory").item(0);
		if (memItem != null) {
			int memItemVal = Integer.valueOf(memItem.getFirstChild()
					.getNodeValue());
			dynamicFormGridPlugin.getItem("memItem").setValue(memItemVal);
		}

		Node jarFile = doc.getElementsByTagName("RestartJarFile").item(0);
		if (jarFile != null) {
			String jarFileVal = jarFile.getFirstChild().getNodeValue();
			dynamicFormGridPlugin.getItem("jarFile").setValue(jarFileVal);
		}

		Node enableMonitored = doc.getElementsByTagName("EnableMonitored")
				.item(0);
		if (enableMonitored != null) {
			Boolean enableMonitoredVal = Boolean.valueOf(enableMonitored
					.getFirstChild().getNodeValue());
			dynamicFormGridPlugin.getItem("enableMonitored").setValue(
					enableMonitoredVal);
		}

		Node gridEngineAdmin = doc.getElementsByTagName("GridEngineAdmin")
				.item(0);
		if (gridEngineAdmin != null) {
			Boolean gridEngineAdminVal = Boolean.valueOf(gridEngineAdmin
					.getFirstChild().getNodeValue());
			dynamicFormGridPlugin.getItem("gridEngineAdmin").setValue(
					gridEngineAdminVal);
		}

		// grid accounting section
		Node gridAcctURL = doc.getElementsByTagName("GridAccountURL").item(0);
		if (gridAcctURL != null) {
			String gridAcctURLVal = gridAcctURL.getFirstChild().getNodeValue();
			dynamicFormGridAccounting.getItem("gridAcctURL").setValue(
					gridAcctURLVal);
		}

		Node gridUsername = doc.getElementsByTagName("GridUsername").item(0);
		if (gridUsername != null) {
			String gridUsernameVal = gridUsername.getFirstChild()
					.getNodeValue();
			dynamicFormGridAccounting.getItem("gridUsername").setValue(
					gridUsernameVal);
		}

		Node gridPassword = doc.getElementsByTagName("GridPassword").item(0);
		if (gridPassword != null) {
			String gridPasswordVal = gridPassword.getFirstChild()
					.getNodeValue();
			dynamicFormGridAccounting.getItem("gridPassword").setValue(
					gridPasswordVal);
		}
	}
